...
edit_ssh_port() {
    ...
    for p in $new_ports; do
        if ! verify_port "ssh" "$p"; then
            echo -e "${RED}❌ Puerto $p ya está en uso por otro servicio.${NC}"
            sleep 2
            return
        fi
    done
    ...
    echo -e "\n${GREEN}[✓] Puertos SSH redefinidos con éxito.${NC}"
}

edit_dropbear_port() {
    if [ ! -f /etc/default/dropbear ]; then
        echo -e "\n${RED}❌ Dropbear no está instalado en este sistema.${NC}"
        sleep 2
        return
    fi
    ...
    echo -e "\n${GREEN}[✓] Puertos Dropbear redefinidos con éxito.${NC}"
}
...
edit_squid_port() {
    ...
    if [ -f /etc/squid/squid.conf ]; then
        ...
        echo -e "\n${RED}❌ Squid Proxy no está instalado en este sistema.${NC}"
        sleep 2
        return
    fi
    ...
    echo -e "\n${GREEN}[✓] Puertos Squid redefinidos con éxito.${NC}"
}
