#!/bin/bash
# MaximusVpsMx - BadVPN UDPGW Manager
...
activar_badvpn() {
    ...
    mkdir -p /etc/MaximusVpsMx/PortM
    echo "$portasx" > /etc/MaximusVpsMx/PortM/Badvpn.log
    ...
    ui_hr
    if [[ -n "$PORT" ]]; then
        echo -e "${GREEN}✓ BADVPN INSTALADO/ACTIVADO CON ÉXITO${NC}"
    else
        echo -e "${RED}❌ No se pudo activar ningún puerto válido.${NC}"
    fi
    ui_pause
}
...
desactivar_badvpn() {
    ...
    rm -f /etc/MaximusVpsMx/PortM/Badvpn.log 2>/dev/null
    echo -e "${GREEN}✓ BADVPN DESINSTALADO / DETENIDO CON ÉXITO${NC}"
    ui_pause
}
