...
ui_header() {
    draw_banner
    ui_hr
    echo -e "           ${YELLOW}ACELERACIÓN TCP BBR / NETSPEED${NC}"
    ui_hr
}
...
start_bbr() {
    ...
    sysctl -p >/dev/null 2>&1
    echo -e "\n${GREEN}[✓] BBR activado exitosamente en el kernel actual.${NC}"
}
...
start_bbrplus() {
    ...
    sysctl -p >/dev/null 2>&1
    echo -e "\n${GREEN}[✓] Aceleración BBRplus activada exitosamente.${NC}"
}
...
install_bbrplus_kernel() {
    ...
    # Descargar versión precompilada del kernel bbrplus para Debian/Ubuntu
    ...
    if [ -f "linux-image-${kernel_ver}.deb" ]; then
        ...
        echo -e "\n${GREEN}[✓] Kernel BBRplus instalado correctamente.${NC}"
        echo -e "${YELLOW}[!] Se requiere reiniciar el VPS para iniciar con el nuevo Kernel.${NC}"
        read -p "¿Deseas reiniciar el VPS ahora? [s/n]: " yn
        if [[ "$yn" == "s" || "$yn" == "S" ]]; then
            reboot
        fi
    else
        echo -e "\n${RED}❌ Error al descargar el Kernel BBRplus. Verifica tu conexión.${NC}"
    fi
}

optimize_system() {
    remove_all
    echo -e "\n${YELLOW}[+] Optimizando límites de archivos y buffers de red...${NC}"
...
*               hard    nofile          1000000" >/etc/security/limits.conf
    ...
    echo -e "\n${GREEN}[✓] Configuración del sistema optimizada correctamente.${NC}"
}
...
while true; do
    ...
    echo -e "  ${CYAN}ACELERACIÓN ACTIVA: ${GREEN}${active_congestion}${NC}"
    ui_subhr
    ...
    echo -e "  ${CYAN}[4]>${WHITE} Optimizar límites del Sistema y Buffers de Red${NC}"
    ...
    case $opt in
        ...
        5) remove_all ; echo -e "\n${GREEN}[✓] Aceleración removida.${NC}" ; ui_pause ;;
        0) exit 0 ;;
        *) continue ;;
    esac
done
