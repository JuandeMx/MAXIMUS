#!/bin/bash
# MaximusVpsMx - Shadowsocks Manager
...
ss_normal_install() {
    ...
    if [[ -f /etc/shadowsocks.json ]]; then
        ...
        echo -e "${GREEN}✓ Shadowsocks Normal desinstalado.${NC}"
        ui_pause
        return 0
    fi
    ...
    echo -e "${WHITE}Elige un método de encriptación:${NC}"
    ...
    read -p "Contraseña para Shadowsocks: " -e -i "maximus" shadowsockspwd
    ...
    # Escribir configuración shadowsocks.json
...
EOF
    ...
    if ps aux | grep -v grep | grep -q "ssserver"; then
        echo -e "${GREEN}✓ SHADOWSOCKS PYTHON INSTALADO CON ÉXITO${NC}"
        local ip_pub
        ip_pub=$(get_public_ip)
        local base_link=$(echo -n "${cipher}:${shadowsockspwd}@${ip_pub}:${shadowsocksport}" | base64 -w0)
        echo -e "\n${YELLOW}📋 ENLACE DE CONEXIÓN:${NC}"
        echo -e "${WHITE}ss://${base_link}${NC}"
    else
        echo -e "${RED}❌ Error al iniciar ssserver. Asegúrese de que no falte Python.${NC}"
    fi
    ui_pause
}
...
ss_libev_install() {
    ...
    if [[ -f /etc/shadowsocks-libev/config.json ]]; then
        ...
        echo -e "${GREEN}✓ Shadowsocks Libev desinstalado.${NC}"
        ui_pause
        return 0
    fi
    ...
    read -p "Contraseña para Shadowsocks Libev: " -e -i "maximus" shadowsockspwd
    
    # Configurar simple-obfs
    read -p "¿Deseas instalar simple-obfs para enmascarar tráfico? [s/n]: " -e -i n simple_obfs_opt
    obfs_plugin=""
    obfs_opts=""
    if [[ "$simple_obfs_opt" == @(s|S|y|Y) ]]; then
        ...
        echo -e "  [1] http"
        echo -e "  [2] tls"
        read -p "Opción [1/2]: " -e -i 1 obfs_mode
        if [[ "$obfs_mode" == "2" ]]; then
...
EOF
    ...
    if ps aux | grep -v grep | grep -q "ss-server"; then
        echo -e "${GREEN}✓ SHADOWSOCKS LIBEV INSTALADO CON ÉXITO${NC}"
        local ip_pub
        ip_pub=$(get_public_ip)
        local base_link=$(echo -n "${cipher}:${shadowsockspwd}@${ip_pub}:${shadowsocksport}" | base64 -w0)
        echo -e "\n${YELLOW}📋 ENLACE DE CONEXIÓN:${NC}"
        ...
        echo -e "${RED}❌ Error al iniciar ss-server. Revise su configuración.${NC}"
    fi
    ui_pause
}
