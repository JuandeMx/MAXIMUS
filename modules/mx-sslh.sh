#!/bin/bash
# MaximusVpsMx - SSLH Multiplexer Manager
...
sslh_inicial() {
    if dpkg -l | grep -q "sslh"; then
        ...
        ui_hr
        echo -e "${GREEN}✓ SSLH DESINSTALADO CON ÉXITO${NC}"
        ui_pause
        return 0
    fi
    
    ui_header "INSTALAR SSLH MULTIPLEXER"
    echo -e "${WHITE}Durante la instalación se abrirá un cuadro azul preguntando el tipo.${NC}"
    echo -e "Selecciona la opción ${YELLOW}standalone${NC} y presiona ENTER para continuar.\n"
    ...
    ui_hr
    echo -e "${GREEN}✓ SSLH INSTALADO CON ÉXITO${NC}"
    ui_pause
}
...
edit_sslh() {
    ...
    if ! dpkg -l | grep -q "sslh"; then
        echo -e "${RED}❌ SSLH no está instalado. Instálalo primero.${NC}"
        ui_pause
        return 1
    fi
    ...
    echo -e "${WHITE}Selecciona el puerto principal donde escuchará SSLH (ej: 443)${NC}"
    while true; do
        read -p "Puerto Principal SSLH: " -e -i "443" SSLHPORT
        if ! mportas | grep -q -w "$SSLHPORT"; then
            break
        else
            echo -e "${RED}❌ El puerto $SSLHPORT ya está ocupado por otro servicio.${NC}"
        fi
    done
    
    ui_hr
    PORTSSHF=""
    read -p "¿Deseas redirigir SSH? [s/n]: " -e -i s ssh_opt
    ...
    PORTSSLF=""
    read -p "¿Deseas redirigir SSL (Stunnel)? [s/n]: " -e -i s ssl_opt
    ...
    PORTOPENVPNF=""
    read -p "¿Deseas redirigir OpenVPN? [s/n]: " -e -i n ovpn_opt
    ...
    AUTOMATICO=""
    read -p "¿Deseas redirigir otro protocolo (Automatico/HTTP)? [s/n]: " -e -i n any_opt
    ...
    ui_hr
    echo -e "${YELLOW}[+] Escribiendo configuración de SSLH...${NC}"
...
EOF
    ...
    if systemctl is-active --quiet sslh 2>/dev/null; then
        echo -e "${GREEN}✓ SSLH INICIADO CON ÉXITO EN EL PUERTO $SSLHPORT${NC}"
    else
        echo -e "${RED}❌ Falló al arrancar SSLH. Verifica la configuración o puerto ocupado.${NC}"
    fi
    ui_pause
}
