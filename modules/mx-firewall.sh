...
save_rules() {
    ...
    # Crear script de restauración al iniciar si no existe
...
block_torrent() {
    echo -e "\n${YELLOW}[+] Aplicando reglas de bloqueo Torrent en el tráfico saliente...${NC}"
    while read -r word; do
        [ -z "$word" ] && continue
        set_keyword_rule "A" "$word"
    done <<<"$bt_keywords"
    save_rules
    echo -e "${GREEN}[✓] Bloqueo de Torrent activado correctamente.${NC}"
}
...
unblock_torrent() {
    ...
    save_rules
    echo -e "${GREEN}[✓] Bloqueo de Torrent desactivado correctamente.${NC}"
}
...
block_spam() {
    ...
    save_rules
    echo -e "${GREEN}[✓] Puertos de correo SPAM bloqueados con éxito.${NC}"
}
...
unblock_spam() {
    ...
    save_rules
    echo -e "${GREEN}[✓] Puertos de correo SPAM desbloqueados con éxito.${NC}"
}
...
block_custom_port() {
    ...
    if [ -n "$custom_port" ]; then
        ...
        save_rules
        echo -e "\n${GREEN}[✓] Puerto $custom_port bloqueado correctamente.${NC}"
    else
        echo -e "\n${RED}Cancelado.${NC}"
    fi
}
...
unblock_custom_port() {
    ...
    if [ -n "$custom_port" ]; then
        ...
        save_rules
        echo -e "\n${GREEN}[✓] Puerto $custom_port desbloqueado correctamente.${NC}"
    else
        echo -e "\n${RED}Cancelado.${NC}"
    fi
}
...
reset_iptables() {
    ...
    save_rules
    echo -e "${GREEN}[✓] Reglas restablecidas con éxito.${NC}"
}
