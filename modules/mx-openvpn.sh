...
install_openvpn() {
    ...
    if [ "$proto" != "udp" ] && [ "$proto" != "tcp" ]; then
        echo -e "${RED}❌ Protocolo inválido.${NC}"; sleep 2; return 1
    fi
...
EOF
    ...
    if systemctl is-active --quiet openvpn-server@server; then
        ...
        echo -e "${RED}❌ OpenVPN no arrancó. Revisa logs con journalctl -u openvpn-server@server${NC}"
    fi
    ui_pause
}
...
EOF
    ...
    echo -e "${GREEN}✓ Perfil creado con éxito: ${WHITE}${client_conf}${NC}"
    ui_pause
}
...
revoke_client() {
    ...
    if ./easyrsa --batch revoke "$client" >/dev/null 2>&1; then
        ...
        echo -e "${GREEN}✓ Cliente $client revocado y archivo .ovpn eliminado.${NC}"
    else
        echo -e "${RED}❌ Error al revocar cliente. Asegúrese de que existe.${NC}"
    fi
    ui_pause
}

edit_ovpn_host() {
    ui_header "CONFIGURACIÓN HOST/DNS OPENVPN"
    echo -e "${WHITE}Este menú permite agregar hosts al archivo /etc/hosts.${NC}"
    ui_hr
    ...
    if [[ -n "$SDNS" ]]; then
        ...
        echo -e "${GREEN}✓ Host agregado a /etc/hosts.${NC}"
    fi
    ui_pause
}
...
toggle_openvpn() {
    ...
    if systemctl is-active --quiet openvpn-server@server; then
        ...
        echo -e "${RED}✓ OpenVPN Detenido.${NC}"
    else
...
uninstall_openvpn() {
    ...
    read -p "¿Estás seguro que deseas eliminar OpenVPN por completo? [s/n]: " conf
    if [[ "$conf" == "s" || "$conf" == "S" ]]; then
        ...
        echo -e "${GREEN}✓ OpenVPN eliminado completamente de su VPS.${NC}"
    fi
    ui_pause
}
