...
# Comprueba la IP pública cada 5 min y actualiza registros A en Cloudflare
...
# Función de log dual (Archivo + Consola interactiva si corresponde)
log_msg() {
    local TYPE="$1"
    local MSG="$2"
...
if [ ! -f "$CONFIG_FILE" ]; then
    log_msg "ERROR" "No existe el archivo de configuración: $CONFIG_FILE"
    exit 1
fi
...
# Si la IP no cambió físicamente, y no se borró el caché manualmente, salir
if [ "$IP_ACTUAL" == "$IP_ANTERIOR" ] && [ -f "$IP_FILE" ]; then
    log_msg "INFO" "La IP pública del VPS no ha cambiado ($IP_ACTUAL). Saliendo."
    exit 0
fi

log_msg "INFO" "Verificando/Actualizando registros en Cloudflare. IP anterior: '$IP_ANTERIOR' → IP actual: '$IP_ACTUAL'"
...
for i in $(seq 0 $(($TOTAL - 1))); do
    ...
    if [ -n "$CF_RECORD_NAME" ]; then
        # Caso 1: Se configuró un dominio específico. Lo actualizamos si el nombre coincide y la IP es distinta.
        ...
        # Caso 2: No se configuró dominio específico. Actualiza los registros que apunten a la IP anterior del VPS.
...
# Guardar la IP actual como referencia local de caché
echo "$IP_ACTUAL" > "$IP_FILE"

log_msg "OK" "Sincronización completada. Se actualizaron $UPDATED de $TOTAL registros tipo A."
