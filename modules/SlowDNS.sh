...
selection_fun() {
    ...
    local select=""
    while true; do
        ui_prompt " Seleccione una opción: "
        read select
...
while true; do
    ...
    # Comprobar si la entrada no está vacía
    if [ -n "$_pub" ]; then
...
# Función para comprobar si una regla existe
rule_exists() {
    local table=$1
    local rule=$2
...
delete_rule() {
    ...
    # Convierte la regla de inserción (-I) en eliminación (-D)
